...
next_state(S, _V, {call, ?M, lookup_messages, _}) ->
    next_now(S);
next_state(S, _V, {call, ?M, purge_single_message, [_, MessID, _]}) ->
    next_now(delete_message(MessID, S)).

precondition(#state{mess_ids=MessIDs},
    {call, ?M, purge_single_message, [_, MessID, _]}) ->
...
delete_message(MessID, S=#state{mess_ids=MessIDs}) when is_integer(MessID) ->
    S#state{mess_ids=lists:delete(MessID, MessIDs)}.
