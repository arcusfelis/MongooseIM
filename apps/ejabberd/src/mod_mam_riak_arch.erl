...
-type riak_connection() :: term().
-type riak_object() :: term().
-type riak_value() :: binary().
-type bucket_name() :: binary().
-type short_key() :: binary().
-type full_key() :: {bucket_name(), short_key()}.
...
purge_multiple_messages(Conn, #jid{lserver=LServer, luser=LUser}, Start, End,
    ...
    undef_jid ->
        ...
        Updated = filter_modified_objects(OldDigestObjects, NewDigestObjects),
        save_updated_digests(Conn, Updated),
        ok;
    bare_jid ->
        ...
        Updated = sparse_purge(Conn, Keys, UserDigestKey)
               ++ filter_modified_objects(OldDigestObjects, NewDigestObjects),
        save_updated_digests(Conn, Updated),
        ok;
    full_jid ->
...
analyse_digest_after(RQ, RSM, Start, End, PageSize, AfterKey, Digest) ->
    ...
    after_middle_page ->
        ...
        RSetDigestCnt = dig_total(RSetDigest),
        TotalCount = StartHourCnt + RSetDigestCnt + RecentCnt,
        UHour = dig_page_maximum_hour(PageSize, PageDigest),
        %% Next message id.
...
analyse_digest_before(RQ, RSM, Start, End, PageSize, BeforeKey, Digest) ->
    ...
    %% S B E
    middle_page ->
        MinHour = dig_page_minimum_hour(PageSize, PageDigest),
        BeforeMessID = key_to_mess_id(BeforeKey),
...
is_empty_digest_obj(Obj) ->
    is_empty_digest_value(riakc_obj:get_value(Obj)).
...
get_actual_digest(Conn, RQ, DigestKey, Now) ->
    ...
    {error, notfound} ->
        {error, notfound};
    {ok, DigestObj} ->
        DigestObj2 = merge_digest_siblings(RQ, DigestObj),
        ...
        true ->
            ...
            has_siblings(DigestObj) andalso
                riakc_pb_socket:put(Conn, DigestObj2),
            {ok, Digest2};
        false ->
            Digest3 = update_digest(RQ, Now, Digest2),
            NewValue = digest_to_binary(Digest3),
            DigestObj3 = riakc_obj:update_value(DigestObj2, NewValue),
            riakc_pb_socket:put(Conn, DigestObj3),
            {ok, Digest3}
        end
    end.

merge_digest_siblings(RQ, DigestObj) ->
...
set_index(MessObj, BUserIDFullRemJIDMessID, BUserIDBareRemJIDMessID) ->
    MD = riakc_obj:get_metadata(MessObj),
    ...
    riakc_obj:update_metadata(MessObj, MD1).

extract_bin_remote_jids(MessObj) ->
    MD = riakc_obj:get_metadata(MessObj),
    [BareSecIdx] = riakc_obj:get_secondary_index(
        MD, user_id_bare_remote_jid_index()),
    [FullSecIdx] = riakc_obj:get_secondary_index(
        MD, user_id_full_remote_jid_index()),
...
merge_and_purge_digest_object(Conn, Start, End, DigestObj) ->
    ...
    MergedDigestObj = merge_digest_siblings(RQ, DigestObj),
    Digest = binary_to_digest(riakc_obj:get_value(MergedDigestObj)),
    NewDigest = purge_digest(Start, End, RQ, Digest),
...
save_updated_digests(Conn, ModifiedObjects) ->
    {Deleted, Updated} =
        lists:partition(fun is_empty_digest_obj/1, ModifiedObjects),
...
-spec to_values(riak_connection(), list(full_key())) -> list(riak_value()).
to_values(Conn, Keys) ->
...
-spec to_objects(riak_connection(), list(full_key())) -> list(riak_object()).
...
get_minimum_key_range_before(RQ, Digest, Before, PageSize) ->
    MinHour = dig_page_minimum_hour(PageSize, Digest),
    LBound = hour_to_min_mess_id(MinHour),
...
dig_add_keys(Keys, Digest) ->
    dig_concat(Digest, dig_new(Keys)).
...
dig_page_minimum_hour(PageSize, Digest) ->
...
dig_page_maximum_hour(PageSize, [{_, Cnt}|Digest = [_|_]])
    when PageSize > Cnt ->
    dig_page_maximum_hour(PageSize - Cnt, Digest);
dig_page_maximum_hour(_, [{Hour, _}|_]) ->
...
dig_between(Start, End, Digest) ->
    ...
    case Strategy of
        ...
        bounded ->
            dig_before_hour(hour(End), dig_after_hour(hour(Start), Digest));
        _ -> Digest
    end.
...
dig_page_minimum_hour_test_() ->
    [?_assertEqual(3, dig_page_minimum_hour( 5, [{1, 6}, {2, 5}, {3, 8}])),
     ?_assertEqual(2, dig_page_minimum_hour(10, [{1, 6}, {2, 5}, {3, 8}])),
     ?_assertEqual(1, dig_page_minimum_hour(20, [{1, 6}, {2, 5}, {3, 8}]))
    ].

-endif.


%% Dirty digest functions
%% -----------------------------------------------------------------------
...
%% @end
purge_digest(Start, End, RQ, Digest) ->
    purge_digest_after(End, RQ, purge_digest_before(Start, RQ, Digest)).

purge_digest_before(undefined, _, Digest) ->
    Digest;
purge_digest_before(Start, RQ, Digest) ->
...
purge_digest_after(undefined, _, Digest) ->
    Digest;
purge_digest_after(End, RQ, Digest) ->
    EndHour = hour(End),
